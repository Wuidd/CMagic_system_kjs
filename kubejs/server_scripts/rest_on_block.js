...
PlayerEvents.tick(event =>{
    ...
    pressureScore.add(Math.ceil(restValue*sleepMulti*timeSpeedMulti*Math.max(1,Math.min(3,6*pressureScore.get()/majo.maxPressure-1))/majo.pressureMulti))
    fatigueScore.add(Math.ceil(restValue*sleepMulti*timeSpeedMulti/(5*majo.fatigueMulti)))
    ...
    if (majo.majolizeFromFaint > 0){
        majo.majolizeFromFaint = majo.majolizeFromFaint-majolizeDiscount*restValue*timeSpeedMulti
    }
})